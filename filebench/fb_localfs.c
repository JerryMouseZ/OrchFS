...
{
	return (pread(fd->fd_num, iobuf, iosize, fileoffset));//pread64
}
...
{
	if ((fd->fd_num = open(path, flags, perms)) < 0) //open64
		return (FILEBENCH_ERROR);
	else
		return (FILEBENCH_OK);
}
...
{
	return (lseek(fd->fd_num, offset, whence));//lseek
}
...
fb_lfs_recur_rm(char *path)
{
	char cmd[MAXPATHLEN];
	//***
	if(*path=='\\'||(path[0]=='/'&&path[1]=='O'&&path[2]=='r'))
		return ;
...
{
	return (pwrite(fd->fd_num, iobuf, iosize, offset));//pwrite64
}
...
fb_lfs_truncate(fb_fdesc_t *fd, off64_t fse_size)
{
	...
	return (ftruncate(fd->fd_num, fse_size));//ftruncate64
#else
